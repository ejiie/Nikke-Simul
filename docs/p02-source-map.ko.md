...
정확한 원본 경로와 SHA-256은 `p02-source-manifest.json`에 기록한다. 원본 프로젝트는 수정하지 않았다.
...
| OL 증가분 계산 | P00에 가져온 `OverloadProcessor.cs` | 원문 유지. P01 normalizedValue를 전달 |
| 스탯 조립 순서 | 기존 `Entities/Nikke.cs` | `CalculationService.cs` 신규 adapter. 각 증가분 추적 추가. 장비·큐브·소장품 기본값 강제 주입을 제거 |
