...
2026-09-10 후속: [P03-C1~C6 연결 보강 완료](p03-connection-completion.ko.md). 아래는 최초 착수 기록이며, 현행 스킬 실행 규칙은 `p03.skills.2`, 실행 연결 계약은 `p03.connection.1`이다.
...
사용자 제공 고정 측정 대상은 **솔로 레이드 챌린지 보스**, 방어력은 **20억 대미지 전 30,925 / 후 31,784**다. **2026-09-11 사용자 재확인: 누적 대미지가 20억을 넘으면 방어력이 실시간 증가하며 피해가 감소하는 현상과 위 수치는 이미 관측·검증 완료**다. 방어력 전환 자체를 다시 실측할 항목으로 두지 않는다. 전투 후 상세 로그·결과 요약·영상은 발당 계산식과 타이밍 대조에 사용한다. 현재 실행기는 요청당 고정 방어력 하나를 사용하므로 각 구간을 따로 검산한다. 전체 전투 재현에는 이 확인된 전환 규칙의 구현이 추가로 필요하다. 관측 범위와 세부 경계 처리는 [측정 조건 안내](p03-measurement-guide.ko.md)를 따른다.
