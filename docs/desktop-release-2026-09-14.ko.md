...
> 이 문서는 **앞선 Director 한정 배포의 역사 기록**이다. 당시 사용자의 원본 실행 경로에는 배포되지 않아 이전 UI가 표시됐다. 이후 원본 main 통합·EXE 재빌드로 정정했다. 현재 사용 경로와 완료 기준은 [원본 배포 기록](desktop-release-original-2026-09-14.ko.md)을 따른다. 아래의 “원본 EXE는 배포 대상이 아니다”는 당시 범위 설명이지 현재 사용 지침이 아니다.
