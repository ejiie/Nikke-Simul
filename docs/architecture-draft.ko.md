...
작성: 2026-09-08. 상태: P00 개발 기반·P01 계정 동기화·P02 스탯 및 단일 히트 검산을 구현했다. [P01 검증 보고서](p01-verification.ko.md)와 [P02 검증 보고서](p02-verification.ko.md)에 검증 범위를 기록했다. 히트 정수화 판정은 추후 실측 대조 대상으로 유지한다. 전체 전투 기능·실전 검증·성능 측정을 완료했다는 의미가 아니다.
...
정적 게임 데이터와 개인 스펙을 별도 snapshot으로 관리한다. 기본 입력은 **내 스펙 동기화 버튼**이며 백엔드가 수집·정제·검증·저장을 수행한다. Python은 로그인 세션과 원천 수집, C# Data 계층은 공통 정제·검증, 로컬 API/SQLite는 작업·snapshot 저장을 담당한다. 기존 getFromBlaLink.py와 upstream HTTP 수집·UI를 재사용하고, 기존 ETL의 필드 매핑을 참고해 손실 없는 adapter를 만든다. CSV는 후순위다.

최초 연결 및 세션 만료 때 로그인하며, 서버·계정 선택을 유지한다. 검증된 수집 결과만 current snapshot으로 전환하고 실패 시 이전 정상 스펙을 보존한다. 비공개·잠금 등 선택 정보는 unknown으로 기록하고 수동 보완 출처를 분리한다. 전체 로스터 수집 완전성과 캐릭터별 계산 가능 여부를 별도로 표시한다. 자세한 상태·저장·구현 순서는 [P01 동기화 설계](p01-sync-design.ko.md)를 따른다. P01에서 실제 로그인·한국 서버 193명 수집·재동기화를 확인했다.
