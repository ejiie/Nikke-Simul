...
public class HitCalculatorTests
{
    ...
    {
        var c = new HitContext { Attack = 100, ChargeApplicable = true, FullCharge = true, ChargeBase = 2.5, ChargeMultiplierBonus = mult, ChargeAdd = add };
    ...
    {
        var c = new HitContext { Attack = 1, Coefficient = 1.9, Crit = true, CritBonus = .5 };
    ...
    {
        var r = HitCalculator.Compare(new() { Attack = 10, AttackDamage = .15, DamageTaken = .15 });
    ...
    {
        var c = new HitContext { Attack = 200, Defense = 100, DamageType = type, DotDamage = .2,
    ...
    {
        var basis = new HitContext { Attack = 100, PierceDamage = .5, PartsDamage = .3, ElementBonus = .4 };
    ...
    {
        Assert.Throws<ArgumentException>(() => HitCalculator.Compare(new() { Attack = 10, CanCrit = false, Crit = true }));
        Assert.Throws<ArgumentException>(() => HitCalculator.Compare(new() { Attack = 10, CanCore = false, Core = true }));
        Assert.Throws<ArgumentException>(() => HitCalculator.Compare(new() { Attack = 10, FullCharge = true }));
        Assert.Throws<ArgumentException>(() => HitCalculator.Compare(new() { Attack = double.NaN }));
        Assert.Throws<ArgumentException>(() => HitCalculator.Compare(new() { Attack = 10 }, 1.5));
    }
    [Fact]
    public void Defense_minimum_is_not_multiplied()
    {
        Assert.All(HitCalculator.Compare(new() { Attack = 10, Defense = 20, Coefficient = 50, AttackDamage = 50 }).Candidates,
