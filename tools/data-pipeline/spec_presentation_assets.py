...
from presentation_assets import ROOT, atomic, download, normal_resource_uri, json_write, digest, PNG
...
def prepare(output):
    ...
    def cached(relative,logical,image=False):
        path=output/relative;url=normal_resource_uri(logical)
        data=path.read_bytes() if path.exists() else download(url)
        if image and not (data.startswith(PNG) or data[:4]==b'RIFF' and data[8:12]==b'WEBP'):raise ValueError('Invalid official image: '+logical)
        if not image:json.loads(data)
    ...
    game=json.loads((ROOT/'data/local/game-catalog.json').read_bytes())
    ...
    result=dict(supportDefinitions=supports,overloadOptions=options,assets=sorted(receipts,key=lambda x:x['path']),source='official Blablalink; Local Lab UI projection; pinned P01 OL value table')
    json_write(output/'spec-presentation.json',result)
    return result

if __name__=='__main__':
    r=prepare(ROOT/'data/local/presentation');print(json.dumps(dict(supports=len(r['supportDefinitions']),options=len(r['overloadOptions']),images=sum('/assets/' in '/'+a['path'] for a in r['assets']))))
