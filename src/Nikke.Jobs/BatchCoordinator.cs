...
public sealed class BatchCoordinator(BatchStore store, HardwareProbe hardware, ExecutionPolicy policy) : IAsyncDisposable
{
    ...
    private readonly ConcurrentDictionary<(string Id,int Attempt),TuningDiagnostics> tuning=new();
    private readonly Queue<(string Id,int Attempt)> recentTuning=new();
    private bool stopped;
    public BatchStatus Status(string id)
    {var status=store.Read(id).Status;return tuning.TryGetValue((id,status.Attempt),out var diagnostic)?status with {Execution=status.Execution with {Tuning=diagnostic}}:status;}
    public async Task<BatchStatus> Create(IPreparedExperiment prepared,ExperimentRequest request,CancellationToken token=default,double? preparationMilliseconds=null)
    {
        ...
        lock(gate){if(stopped)throw new InvalidOperationException("compute_stopped");if(active.Count>=4)throw new InvalidOperationException("compute_queue_full");
            var batch=store.Create(prepared,request,selection);Schedule(batch,prepared,preparationMilliseconds);return batch;}
    }
    ...
    public BatchStatus Resume(string id,IPreparedExperiment prepared,double? preparationMilliseconds=null)
    {lock(gate){if(stopped)throw new InvalidOperationException("compute_stopped");if(active.Count>=4)throw new InvalidOperationException("compute_queue_full");if(active.ContainsKey(id))throw new InvalidOperationException("attempt_still_active");
        ...
        var batch=store.Resume(id);Schedule(batch,prepared,preparationMilliseconds);return batch;}}
    private void Schedule(BatchStatus batch,IPreparedExperiment prepared,double? preparationMilliseconds)
    {
        var token=new CancellationTokenSource();
        // Start under gate so a very short batch cannot finish before it is registered.
        var task=Task.Run(async()=>{lock(gate){} await Execute(batch,prepared,token,preparationMilliseconds);});
        active[batch.Id]=(token,task);
    }
    private async Task Execute(BatchStatus batch,IPreparedExperiment prepared,CancellationTokenSource cancellation,double? preparationMilliseconds)
    {
        ...
        try {
            ...
            var selection=await policy.Select(await hardware.Detect(cancellation.Token),prepared,request.Execution??new(),cancellation.Token,
                diagnostic=>tuning[(batch.Id,batch.Attempt)]=diagnostic,preparationMilliseconds);
            ...
            lock(gate){active.TryRemove(batch.Id,out _);cancellation.Dispose();recentTuning.Enqueue((batch.Id,batch.Attempt));
                while(recentTuning.Count>64)tuning.TryRemove(recentTuning.Dequeue(),out _);}
        }
    }
