...
**현재 P02 단일 히트 검산 구현 완료:** P01 계정 동기화에 C# 스탯 분해·확정 차지식·정수화 후보 비교·실측 입력·검산 JSON 저장을 연결했습니다. 히트 정수화는 미확정이며 스킬 시간축·팀 전투는 P03 이후입니다.
...
기존 C#의 스탯 계산·테이블·관련 DTO는 바이트 그대로 보존합니다. P01 수집기·정제기는 양쪽 원본의 요청 구조·매핑을 참고해 새 계약으로 작성했고, UI는 upstream 테마 일부를 재사용했습니다. P02에서는 캐릭터 선택 후 `스탯 계산` → 타격 조건 입력 → `정수화 후보 비교`로 검산합니다. 계수는 % 단위로 입력합니다. 스킬 계수·버프·허용 조건은 직접 지정하며 캐릭터 스킬을 자동 적용하지 않습니다.
