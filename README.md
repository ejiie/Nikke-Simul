...
**현재 P01 계정 동기화 구현 완료:** 버튼 기반 로그인·수집·정제·검증·SQLite 저장·스펙 확인·수동 보완. 실제 한국 서버 193명 수집과 원천 대조를 확인했습니다. 새 C# 전투 엔진 연결은 P02 이후입니다.
...
기존 C#의 `OverloadProcessor`와 `OverloadOptionDto` 두 파일은 바이트 그대로 보존합니다. P01 수집기·정제기는 양쪽 원본의 요청 구조·매핑을 참고해 새 계약으로 작성했고, UI는 upstream 테마 일부를 재사용했습니다. 전체 기초 스탯·차지·히트 대미지 이식은 P02에서 진행합니다.
