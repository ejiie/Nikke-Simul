...
**2026-09-18 수치 정밀도 후속 결정:** `long` 기반 고정소수점으로 전환하고, 스노우 화이트 풀차지 버스트의 단일 타격 계산 과정에서 중간값·최종 피해 오버플로를 검사할 예정입니다. [결정·완료 조건](docs/p02-buff-correction.ko.md). 현재는 문서 기록만 완료했으며 코드 전환·검사·배포는 미실행입니다.
