...
**P03 진행 중:** 리타·블랑·누아르·앨리스·모더니아의 공식 스킬 효과를 시간축에 연결했습니다. 실제 계정 스킬 레벨로 버프·추가타·무기 변경을 실행하고 효과별 결과를 API로 저장합니다. [스킬 구현 범위·출처](docs/p03-skill-runtime.ko.md) · [진행 기록](docs/p03-progress.ko.md) · [틱 대미지 측정 조건](docs/p03-measurement-guide.ko.md). 현재 버스트 시전과 풀버스트 구간은 지정 조건이며 실제 팀 게이지·보스 기믹·UI 연결·실측 대조는 후속 작업입니다.
